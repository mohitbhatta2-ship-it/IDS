#!/usr/bin/env python3
"""
Standalone verifier for the TARGETED benign real-PCAP corpus (no Django, no model).
...
controlled-local only (127.0.0.0/8 or the host's own eth0 IP -- never external),
TCP only, finite duration; plus manifest<->disk agreement and no duplicate PCAPs
(by content hash). Exits non-zero on any failure. Labels are read from the
manifest (scenario/folder), never inferred from a model.

    python3 validation/targeted_benign_pcaps/verify_pcaps.py [dir]
"""
...
def main(argv):
    ...
    hashes, dups, n_ok, missing = {}, [], 0, []
    print(f"Verifying {len(rows)} captures under {root} (host eth0={eth0})\n")
    print(f"{'capture':12} {'scenario':30} {'host':11} {'port':>5} {'pkts':>5} {'dur_s':>7}  status")
    for r in rows:
        matches = list((root / "benign").glob(f"{r['capture_id']}_*.pcap"))
        if not matches:
            missing.append(r["capture_id"]); print(f"{r['capture_id']:12} MISSING"); continue
        pcap = matches[0]
        ...
        print(f"{r['capture_id']:12} {r['scenario'][:30]:30} {host:11} {port:5} {res['packets']:5} "
